...
#define VECTOR_SIZE (FILTER_SIZE*FILTER_SIZE*INPUT_CHANS)
...
// above P times.
#define INTERLEAVE 4 // INTERLEAVE must be at least the latency of the floating point accumulation.
#define PSUM_LEN (INTERLEAVE*SCALE_FACTOR)
void dot_product (
   data_t ifmap_vec[VECTOR_SIZE],
   data_t weight_vec[VECTOR_SIZE],
...
data_t accum (data_t psum_vec[PSUM_LEN]) {
   data_t sum = 0.0;
...
void get_next_ijk (int indices[3]) {
   static int i = 0;
   static int j = 0;
   static int k = 0;
   indices[0] = i;
...
void conv (
   ...
   TOP_LOOP: for (int f = 0; f < (OUTPUT_HEIGHT * OUTPUT_WIDTH * OUTPUT_CHANS); f++) {
      ...
      data_t psum_vec[PSUM_LEN];
      int indices[3];
      get_next_ijk(indices);
      int i_int = indices[0];
      int j_int = indices[1];
      int k_int = indices[2];
      // FOR EACH OUTPUT ELEMENT:
      // read the inputs needed for this output element
      readInputs(in_data, i_int, j_int, ifmap_vec);
      // read the filters needed for this output element
      readFilters(filter_data, k_int, weight_vec);
      // Technically we can do one of the above just once but it shouldn't affect performance.
      // Calculate the dot product, result is returned as a list of partial sums
      dot_product(ifmap_vec, weight_vec, psum_vec);
      // Accumulate the partial sums and then store the output.
      out_data[f] = accum(psum_vec);
   }
}
