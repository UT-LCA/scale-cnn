###############################################################################
#
#  tdf5_conv_directives.tcl  -- AUTO-GENERATED --
#
#  Directives used to synthesize tdf5 that are common regardless of
...
if {$OCHAN_SCALE_FACTOR > 1} {
   ...
   set_directive_array_partition -type cyclic -factor $OCHAN_SCALE_FACTOR -dim 1 tdf5 weight_vecs
   set_directive_array_partition -type cyclic -factor $OCHAN_SCALE_FACTOR -dim 1 tdf5 products
}

if {$READ_SCALE_FACTOR > 1} {
   set_directive_array_partition -type cyclic -factor $READ_SCALE_FACTOR -dim 3 tdf5 ifmap_vec
   set_directive_array_partition -type cyclic -factor $READ_SCALE_FACTOR -dim 4 tdf5 weight_vecs
   set_directive_array_partition -type cyclic -factor $PRODUCTS_PART_FACTOR -dim 2 tdf5 products
}
...
if {$READ_SCALE_FACTOR < 16} {
   ...
   set_directive_pipeline tdf5_readInputs/IL6
   set_directive_unroll -factor $unroll tdf5_readInputs/IL6
} elseif {$READ_SCALE_FACTOR == 16} {
   set_directive_pipeline tdf5_readInputs/IL5
} else {
...
if {$READ_SCALE_FACTOR < 16} {
   set_directive_pipeline tdf5_readFilters/FL6
   set_directive_unroll -factor $READ_SCALE_FACTOR tdf5_readFilters/FL6
} elseif {$READ_SCALE_FACTOR == 16} {
   set_directive_pipeline tdf5_readFilters/FL5
}

# Pipeline and unroll dot product multiplications
if {$READ_SCALE_FACTOR < 16} {
   set_directive_pipeline tdf5_dot_product/DP_OUTER_3
   set_directive_unroll -factor $READ_SCALE_FACTOR tdf5_dot_product/DP_OUTER_3
} elseif {$READ_SCALE_FACTOR == 16} {
   set_directive_pipeline tdf5_dot_product/DP_OUTER_2
}

# Unroll the adjustment loop if appropriate
if {$UNROLL_ADJUST_LOOP} {
   set_directive_unroll -factor 2 tdf5_adjust/ADJUST_LOOP
}

# Use dataflow pipelining for the top-level loop.
set_directive_dataflow tdf5/TOP_LOOP
