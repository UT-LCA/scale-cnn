...
// It reduces 288 inputs to 18 outputs.
...
void tdf8_accum_1(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 9; i1++) {
      ...
      accum_out[out_idx+0] = sum29;
      accum_out[out_idx+1] = sum28;
      out_idx += 2;
...
// This is a pipelined tree accumulation stage
// It reduces 18 inputs to 5 outputs.
...
void tdf8_accum_2(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 5; i1++) {
      ...
      WRPC_LOOP: for (uint16_t w = 0; w < 4; w++) {
         // Need this bounds check because input length is not necessarily
         // a multiple of words read per cycle.
         vals[w] = (i+w < 18) ? accum_in[i+w] : (data_t)0;
      }
...
data_t tdf8_accum_3(data_t accum_in[5]) {
   data_t sum = 0.0;
...
void tdf8 (
   ...
   TOP_LOOP: for (int f = 0; f < TOP_LOOP_ITERATIONS; f++) {
      ...
      data_t accum1_out_0[18];
      data_t accum1_out_1[18];
      data_t accum1_out_2[18];
      data_t accum1_out_3[18];
      data_t accum1_out_4[18];
      data_t accum1_out_5[18];
      data_t accum1_out_6[18];
      data_t accum1_out_7[18];
