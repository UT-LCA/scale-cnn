...
// It reduces 144 inputs to 36 outputs.
...
void tdf2_accum_1(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 9; i1++) {
      ...
      accum_out[out_idx+0] = sum11;
      accum_out[out_idx+1] = sum10;
      accum_out[out_idx+2] = sum9;
      accum_out[out_idx+3] = sum8;
      out_idx += 4;
...
// This is a pipelined tree accumulation stage
// It reduces 36 inputs to 10 outputs.
...
void tdf2_accum_2(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 5; i1++) {
      ...
      WRPC_LOOP: for (uint16_t w = 0; w < 8; w++) {
         // Need this bounds check because input length is not necessarily
         // a multiple of words read per cycle.
         vals[w] = (i+w < 36) ? accum_in[i+w] : (data_t)0;
      }
      ...
      accum_out[out_idx+0] = sum5;
      accum_out[out_idx+1] = sum4;
      out_idx += 2;
...
// This is a pipelined tree accumulation stage
// It reduces 10 inputs to 3 outputs.
...
void tdf2_accum_3(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 3; i1++) {
      ...
      WRPC_LOOP: for (uint16_t w = 0; w < 4; w++) {
         // Need this bounds check because input length is not necessarily
         // a multiple of words read per cycle.
         vals[w] = (i+w < 10) ? accum_in[i+w] : (data_t)0;
      }
...
// It reduces 3 inputs to 1 output.
// The estimated latency is 17 cycles.
data_t tdf2_accum_4(data_t accum_in[3]) {
   data_t sum0 = accum_in[2] + accum_in[1];
   data_t sum1 = accum_in[0] + sum0;
   return sum1;
...
void tdf2 (
   ...
   TOP_LOOP: for (int f = 0; f < TOP_LOOP_ITERATIONS; f++) {
      ...
      data_t accum1_out_0[36];
      data_t accum1_out_1[36];
      data_t accum1_out_2[36];
      data_t accum1_out_3[36];
      ...
      #pragma HLS array_partition variable=accum1_out_2 cyclic factor=4
      #pragma HLS array_partition variable=accum1_out_3 cyclic factor=4
      ...
      data_t accum2_out_0[10];
      data_t accum2_out_1[10];
      data_t accum2_out_2[10];
      data_t accum2_out_3[10];
      ...
      data_t accum3_out_0[3];
      data_t accum3_out_1[3];
      data_t accum3_out_2[3];
      data_t accum3_out_3[3];
      ...
      tdf2_accum_3(accum2_out_2, accum3_out_2);
      tdf2_accum_3(accum2_out_3, accum3_out_3);
      sums[0] = tdf2_accum_4(accum3_out_0);
      sums[1] = tdf2_accum_4(accum3_out_1);
      sums[2] = tdf2_accum_4(accum3_out_2);
      sums[3] = tdf2_accum_4(accum3_out_3);
