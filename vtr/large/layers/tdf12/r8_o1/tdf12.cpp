...
// It reduces 128 inputs to 64 outputs.
...
void tdf12_accum_1(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 8; i1++) {
      ...
      accum_out[out_idx+0] = sum7;
      accum_out[out_idx+1] = sum6;
      accum_out[out_idx+2] = sum5;
      accum_out[out_idx+3] = sum4;
      accum_out[out_idx+4] = sum3;
      accum_out[out_idx+5] = sum2;
      accum_out[out_idx+6] = sum1;
      accum_out[out_idx+7] = sum0;
      out_idx += 8;
...
// The estimated latency is 23 cycles.
void tdf12_accum_2(
   data_t accum_in[64], 
   data_t accum_out[16]
) {
