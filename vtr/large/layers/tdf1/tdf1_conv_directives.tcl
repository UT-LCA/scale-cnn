###############################################################################
#
#  tdf1_conv_directives.tcl  -- AUTO-GENERATED --
#
#  Directives used to synthesize tdf1 that are common regardless of
...
if {$OCHAN_SCALE_FACTOR > 1} {
   ...
   set_directive_array_partition -type cyclic -factor $OCHAN_SCALE_FACTOR -dim 1 tdf1 weight_vecs
   set_directive_array_partition -type cyclic -factor $OCHAN_SCALE_FACTOR -dim 1 tdf1 products
}

if {$READ_SCALE_FACTOR > 1} {
   set_directive_array_partition -type cyclic -factor $READ_SCALE_FACTOR -dim 3 tdf1 ifmap_vec
   set_directive_array_partition -type cyclic -factor $READ_SCALE_FACTOR -dim 4 tdf1 weight_vecs
   set_directive_array_partition -type cyclic -factor $PRODUCTS_PART_FACTOR -dim 2 tdf1 products
}
...
if {$READ_SCALE_FACTOR < 3} {
   set unroll $READ_SCALE_FACTOR
   set_directive_pipeline tdf1_readInputs/IL6
   set_directive_unroll -factor $unroll tdf1_readInputs/IL6
} elseif {$READ_SCALE_FACTOR == 3} {
   set_directive_pipeline tdf1_readInputs/IL5
} else {
...
if {$READ_SCALE_FACTOR < 3} {
   set_directive_pipeline tdf1_readFilters/FL6
   set_directive_unroll -factor $READ_SCALE_FACTOR tdf1_readFilters/FL6
} elseif {$READ_SCALE_FACTOR == 3} {
   set_directive_pipeline tdf1_readFilters/FL5
}

# Pipeline and unroll dot product multiplications
if {$READ_SCALE_FACTOR < 3} {
   set_directive_pipeline tdf1_dot_product/DP_OUTER_3
   set_directive_unroll -factor $READ_SCALE_FACTOR tdf1_dot_product/DP_OUTER_3
} elseif {$READ_SCALE_FACTOR == 3} {
   set_directive_pipeline tdf1_dot_product/DP_OUTER_2
}

# Unroll the adjustment loop if appropriate
if {$UNROLL_ADJUST_LOOP} {
   set_directive_unroll -factor 2 tdf1_adjust/ADJUST_LOOP
}

# Use dataflow pipelining for the top-level loop.
set_directive_dataflow tdf1/TOP_LOOP
