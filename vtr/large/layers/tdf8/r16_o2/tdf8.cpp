...
// It reduces 288 inputs to 72 outputs.
...
// This is a pipelined tree accumulation stage
// It reduces 72 inputs to 20 outputs.
...
void tdf8_accum_2(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 5; i1++) {
      ...
      WRPC_LOOP: for (uint16_t w = 0; w < 16; w++) {
         // Need this bounds check because input length is not necessarily
         // a multiple of words read per cycle.
         vals[w] = (i+w < 72) ? accum_in[i+w] : (data_t)0;
      }
...
// This is a pipelined tree accumulation stage
// It reduces 20 inputs to 6 outputs.
...
void tdf8_accum_3(
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 3; i1++) {
      ...
      WRPC_LOOP: for (uint16_t w = 0; w < 8; w++) {
         // Need this bounds check because input length is not necessarily
         // a multiple of words read per cycle.
         vals[w] = (i+w < 20) ? accum_in[i+w] : (data_t)0;
      }
...
// It reduces 6 inputs to 1 output.
// The estimated latency is 26 cycles.
data_t tdf8_accum_4(data_t accum_in[6]) {
   data_t sum0 = accum_in[5] + accum_in[4];
   data_t sum1 = accum_in[3] + accum_in[2];
   data_t sum2 = accum_in[1] + accum_in[0];
...
void tdf8 (
   ...
   TOP_LOOP: for (int f = 0; f < TOP_LOOP_ITERATIONS; f++) {
      ...
      data_t accum1_out_0[72];
      data_t accum1_out_1[72];
      ...
      data_t accum2_out_0[20];
      data_t accum2_out_1[20];
      ...
      data_t accum3_out_0[6];
      data_t accum3_out_1[6];
