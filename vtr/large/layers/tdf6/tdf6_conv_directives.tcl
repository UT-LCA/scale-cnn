###############################################################################
#
#  tdf6_conv_directives.tcl  -- AUTO-GENERATED --
#
#  Directives used to synthesize tdf6 that are common regardless of
...
if {$OCHAN_SCALE_FACTOR > 1} {
   ...
   set_directive_array_partition -type cyclic -factor $OCHAN_SCALE_FACTOR -dim 1 tdf6 weight_vecs
   set_directive_array_partition -type cyclic -factor $OCHAN_SCALE_FACTOR -dim 1 tdf6 products
}

if {$READ_SCALE_FACTOR > 1} {
   set_directive_array_partition -type cyclic -factor $READ_SCALE_FACTOR -dim 3 tdf6 ifmap_vec
   set_directive_array_partition -type cyclic -factor $READ_SCALE_FACTOR -dim 4 tdf6 weight_vecs
   set_directive_array_partition -type cyclic -factor $PRODUCTS_PART_FACTOR -dim 2 tdf6 products
}
...
if {$READ_SCALE_FACTOR < 128} {
   ...
   set_directive_pipeline tdf6_readInputs/IL6
   set_directive_unroll -factor $unroll tdf6_readInputs/IL6
} elseif {$READ_SCALE_FACTOR == 128} {
   set_directive_pipeline tdf6_readInputs/IL5
} else {
...
if {$READ_SCALE_FACTOR < 128} {
   set_directive_pipeline tdf6_readFilters/FL6
   set_directive_unroll -factor $READ_SCALE_FACTOR tdf6_readFilters/FL6
} elseif {$READ_SCALE_FACTOR == 128} {
   set_directive_pipeline tdf6_readFilters/FL5
}

# Pipeline and unroll dot product multiplications
if {$READ_SCALE_FACTOR < 128} {
   set_directive_pipeline tdf6_dot_product/DP_OUTER_3
   set_directive_unroll -factor $READ_SCALE_FACTOR tdf6_dot_product/DP_OUTER_3
} elseif {$READ_SCALE_FACTOR == 128} {
   set_directive_pipeline tdf6_dot_product/DP_OUTER_2
}

# Unroll the adjustment loop if appropriate
if {$UNROLL_ADJUST_LOOP} {
   set_directive_unroll -factor 2 tdf6_adjust/ADJUST_LOOP
}

# Use dataflow pipelining for the top-level loop.
set_directive_dataflow tdf6/TOP_LOOP
