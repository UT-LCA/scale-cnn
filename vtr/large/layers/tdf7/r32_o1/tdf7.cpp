...
// It reduces 288 inputs to 20 outputs.
// The estimated latency is 42 cycles.
void tdf7_accum_1(
   data_t accum_in[288], 
   data_t accum_out[20]
) {
   ...
   IL_LOOP: for (uint16_t i1 = 0; i1 < 5; i1++) {
      ...
      data_t sum48 = sum32 + sum33;
      data_t sum49 = sum34 + sum35;
      data_t sum50 = sum36 + sum37;
      data_t sum51 = sum38 + sum39;
      data_t sum52 = sum40 + sum41;
      data_t sum53 = sum42 + sum43;
      data_t sum54 = sum44 + sum45;
      data_t sum55 = sum46 + sum47;
      data_t sum56 = sum48 + sum49;
      data_t sum57 = sum50 + sum51;
      data_t sum58 = sum52 + sum53;
      data_t sum59 = sum54 + sum55;
      accum_out[out_idx+0] = sum59;
      accum_out[out_idx+1] = sum58;
      accum_out[out_idx+2] = sum57;
      accum_out[out_idx+3] = sum56;
      out_idx += 4;
...
// This is a pipelined tree accumulation stage
// It reduces 20 inputs to 3 outputs.
...
data_t tdf7_accum_3(data_t accum_in[3]) {
   data_t sum = 0.0;
   for (int i = 0; i < 3; i++) sum += accum_in[i];
   return sum;
}
