...
class AccumGenerator():

   def __init__(this, targ_lat, hadd_lat, ochan_sf):
      this.target_latency = targ_lat
      this.hadd_latency = hadd_lat
      this.ochan_sf = ochan_sf
   ...
   def PipelinedTreeStageNoncomplete(this, curr_IL, input_read_bw):
      ...
      max_tree_height_latency_limited = math.floor((this.target_latency - trip_count - total_overhead) / this.hadd_latency)
      ...
      latency = (this.hadd_latency*tree_height) + trip_count + total_overhead
      ...
      # It could be possible to just partition it half the amount, but a larger read bandwidth for the next
      # stage might mean fewer accumulation stages in total. It's hard to objectively decide whether or not
      # this is a good idea.
   ...
   def PipelinedTreeStageComplete(this, curr_IL):
      ...
      for wrpc in range(2, curr_IL+2, 2):
         ...
         for th in range(1, math.ceil(math.log2(wrpc))+1):
            latency = (this.hadd_latency*th) + trip_count + OVERHEAD
      ...
      # would just have a bunch of stages where each one has just a single adder.
      #
      # So to strike a balance, I will use this heuristic: Choose the design point with the smallest
      # output length, that uses each DSP at least 4 times per function call. If no such point exists,
   ...
   def InterleavedStage(this, curr_IL, input_read_bw):
      accum_stage = {}
      ...
      stage_overhead = 3
      part_factor = math.ceil((ol/2) / (this.target_latency - loop1_total_latency - stage_overhead))
   ...
   def UnpipelinedTreeStage(this, curr_IL):
      accum_stage = {}
   ...
   def GetAccumulationStageParams(this, input_length, input_read_bw):
      ...
      while (curr_IL > 1):
         if (this.hadd_latency * curr_IL) + OVERHEAD < this.target_latency:
            ...
            # last stage.
            accum_stage = this.SimpleLoopStage(curr_IL)
         elif curr_input_read_bw >= curr_IL and curr_IL <= 8 and (this.hadd_latency * math.ceil(math.log2(curr_IL)) - 1 < this.target_latency):
            ...
            accum_stage = this.UnpipelinedTreeStage(curr_IL)
            ...
            accum_stage = this.InterleavedStage(curr_IL, curr_input_read_bw)
         else:
            ...
            if curr_input_storage_type == 'bram':
               accum_stage = this.PipelinedTreeStageNoncomplete(curr_IL, curr_input_read_bw)
            else:
               accum_stage = this.PipelinedTreeStageComplete(curr_IL)
   ...
   def GenUnpipelinedTreeAccumStage(this, stage):
   ...
   def GenInterleavedAccumStage(this, stage):
   ...
   def GenerateAccumStageCode(this, stage):
      if stage['type'] == 'simple_loop':
         func = this.GenSimpleLoopAccumStage(stage)
      elif stage['type'] == 'interleaved':
         func = this.GenInterleavedAccumStage(stage)
      elif stage['type'] == 'pipelined_tree':
         func = this.GenPipelinedTreeAccumStage(stage)
      elif stage['type'] == 'unpipelined_tree':
         func = this.GenUnpipelinedTreeAccumStage(stage)
      else:
         raise Exception('Unknown accum stage type.')
      return func

   # Generates the code that calls all of the accumulation stages in sequence.
   def GenAccumStageFuncCalls(this, lname, ltype, stage_params):
      code = ""
      s = " " * 6
      ...
      for stage in stage_params:
         ...
         if has_return_val:
            ...
            for ochan in range(this.ochan_sf):
   ...
   def GenerateAccumulationStages(this, layer_name, layer_type, input_length, read_bw):
      stage_params = this.GetAccumulationStageParams(input_length, read_bw)
      stage_functions = []
      for stage in stage_params:
         stage['lname'] = layer_name
         stage_functions.append(this.GenerateAccumStageCode(stage))
      function_defs_code  = '\n\n\n'.join(stage_functions)
      function_calls_code = this.GenAccumStageFuncCalls(layer_name, layer_type, stage_params)
